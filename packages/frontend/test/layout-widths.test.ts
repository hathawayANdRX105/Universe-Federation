...
describe('wide layout sizing', () => {
	test('keeps the primary desktop columns balanced and adaptive', () => {
		...
		assert.match(universalSource, /universal-nonTitlebarArea-[^}]+max-width:\s*1360px\s*!important;/);
		assert.match(universalSource, /universal-nonTitlebarArea-[^}]+align-self:\s*center\s*!important;/);
