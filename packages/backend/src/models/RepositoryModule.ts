...
const $chatRoomUserMutingsRepository: Provider = {
	provide: DI.chatRoomUserMutingsRepository,
	useFactory: (db: DataSource) => db.getRepository(MiChatRoomUserMuting).extend(miRepository as MiRepository<MiChatRoomUserMuting>),
	inject: [DI.db],
};

const $chatRoomBanningsRepository: Provider = {
	provide: DI.chatRoomBanningsRepository,
	useFactory: (db: DataSource) => db.getRepository(MiChatRoomBanning).extend(miRepository as MiRepository<MiChatRoomBanning>),
	inject: [DI.db],
};
...
const $noteSentimentsRepository: Provider = {
	provide: DI.noteSentimentsRepository,
	useFactory: (db: DataSource) => db.getRepository(MiNoteSentiment).extend(miRepository as MiRepository<MiNoteSentiment>),
	inject: [DI.db],
};
